    ...
    try:
        ...
        for inputDB in dbList:
            ...
            for txtFile in txtFiles:
                ...
                arcpy.SetProgressorLabel("Importing " +  fnAreasymbol + " tabular data  (" + Number_Format(iCntr, 0, True) + " of " + Number_Format(len(dbList), 0, True) + ") :   " + tblName)
                ...
                if arcpy.Exists(tbl):
                    ...
                    if not tblName in ['sdvfolderattribute', 'sdvattribute', 'sdvfolder', 'sdvalgorithm']:
                        # Import all tables except SDV
                        #
                        time.sleep(0.05)  # Occasional write errors

                        if tblName.endswith("text"):
                        ...
                        elif tblName == "cointerp":
                            # FY2020 we started skipping certain columns and rows in cointerp that aren't currently
                            # being used. The rows being skipped contain interp rating reasons.
                                ...
                                if arcpy.Exists(txtPath):
                                    ...
                                    try:
                                        ...
                                        for rowInFile in csv.reader(open(txtPath, 'rb'), delimiter='|', quotechar='"'):
                                            ...
                                            r1.extend(r2)
                                            r1.extend(r3)
                                            fixedRow = [x if x else None for x in r1]  # substitue Null values for no data

                                            if (fixedRow[6] == '0') or (fixedRow[1] == '54955'):
                                                # Skip NCCPI or ruledepth = zero (reasons for interp ratings)
                                                # should I make the 54955 a dynamic variable? Bob says he has no plans to change NCCPI v3.
                                                cursor.insertRow(fixedRow) # was fixedRow
                            ...
                            if os.path.isfile(txtPath):
                                ...
                                try:
                                    ...
                                    for rowInFile in csv.reader(open(txtPath, 'rb'), delimiter='|', quotechar='"'):
                                        newRow = list()
                                        fldNo = 0
                                        keyVal = int(rowInFile[dIndex[tblName]])
                    ...
                    raise MyError, "Required table '" + tblName + "' not found in " + newDB
            ...
            if arcpy.Exists(txtPath):
                ...
                with arcpy.da.InsertCursor(tbl, fldNames) as cursor:
                    # counter for current record number
                    iRows = 1
                    arcpy.SetProgressorLabel("Importing " +  fnAreasymbol + "  (" + Number_Format(iCntr, 0, True) + " of " + Number_Format(len(dbList), 0, True) + "):   " + tblName)
...
def gSSURGO(inputFolder, surveyList, outputWS, AOI, tileInfo, useTextFiles, bClipped, areasymbolList):
    ...
    try:
        ...
        #codePage = 'iso-8859-1'  # allow csv reader to handle non-ascii characters. failing on xd7 latin plant names
        codePage = 'cp1252'  # allow csv reader to handle non-ascii characters
        ...
        if len(mupolyList) > 0:
            ...
            arcpy.SetProgressorLabel("Successfully created new database...")
...
try:
    if __name__ == "__main__":
        ...
        for feature in features:
            PrintMsg("\nRemoving feature class: " + feature + " from the rss geodatabase.")
